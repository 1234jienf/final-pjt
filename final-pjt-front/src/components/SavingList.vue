<template>
  <div class="main">
    ...
    <select v-model="selectedBank" @change="filterSavings">
    ...
    <select v-model="selectedSaveTerm" @change="filterSavings">
...
const filterSavings = async () => {
  try {
    ...
    if (!selectedBank.value || !selectedSaveTerm.value) {
      savingOptions.value = await store.getSavings();
      return;
    }

    const response = await axios.get(`${store.API_URL}/finlife/saving-sort/${bankId}/${saveTerm}`, {
      headers: {
